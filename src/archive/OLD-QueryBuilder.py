...
class QueryBuilder():
    ...
    # --------------------------
    def get_update_query_with_bindings(self,table_name:str,items:dict,where_option:dict={}):
        '''UPDATE 테이블명 SET 키1 = 값1, 키2 = 값2 ... WHERE 조건;,bindings'''
        where_str,where_bindings = self._get_where_str_with_bindings(**where_option) if where_option else ('',[])
    ...
    # --------------------------
    def get_delete_query_with_bindings(self,table_name:str,where_option:dict={}):
        '''DELETE FROM 테이블명 WHERE 조건'''
        where_str,bindings = self._get_where_str_with_bindings(**where_option) if where_option else ('',[])
    ...
    def get_delete_query_by_item(self,table_name:str,items:dict):
        ...
        return f'''DELETE FROM "{table_name}" {where_str};'''
    # --------------------------
    def get_delete_query_by_item_with_bindings(self,table_name:str,items:dict):
        '''DELETE FROM 테이블명 WHERE 조건 - 모두 일치하는 항목 삭제'''
...
if __name__ == '__main__':
    ...
    # print(res)
    # res = qb.get_update_query_with_bindings('users',{'name':'Tom','age':23},{'likepattern':[('city','S%')]})
    # print(res)
    # res = qb.get_delete_query('users',{'comparison':[('name','=','Tom')]})
    # print(res)
    # res = qb.get_delete_query_with_bindings('users',{'comparison':[('name','=','Tom')]})
    # print(res)
    # res = qb.get_delete_query_by_item('users',{'name':'Tom','city':'busan','age':24})
    # print(res)
    # print('# ------------------------------------------')
    # res = qb.get_delete_query_by_item_with_bindings('users',{'name':'Tom','city':'busan','age':24})
    # print(res)
